...
SLIDING_MOVES_TABLE: list[int] = [0 for _ in range(1 << 18)] + [0 for _ in range(1 << 15)]

from math import log2

CAP_TO_64_BITS = lambda N: N & (1 << 64) - 1

bit_index = lambda N: int(log2(N))

# Instantiate rook moves
for squarePosition in range(64):
    squares = []
    entry = ROOK_MAGICS[squarePosition]
    ...
    for linearOccupation in range(1 << len(squares)):
        ...
        hash = CAP_TO_64_BITS(mappedOccupancy * entry.magic) >> entry.shift

        SLIDING_MOVES_TABLE[entry.offset + hash] = traceOutRookMoves(mappedOccupancy, squarePosition)

BISHOPS_START_FROM = 1 << 18

# Instantiate bishop moves
for squarePosition in range(64):
    squares = []
    entry = BISHOP_MAGICS[squarePosition]
    ...
    for linearOccupation in range(1 << len(squares)):
        ...
        SLIDING_MOVES_TABLE[BISHOPS_START_FROM + entry.offset + hash] = traceOutBishopMoves(mappedOccupancy, squarePosition)



def get_rook_moves(bitboard: int, square: int) -> int:
    entry = ROOK_MAGICS[square]
    
    occ = bitboard & entry.mask
    occ *= entry.magic
    occ &= (1 << 64) - 1
    
    index = (occ >> entry.shift) + entry.offset

    return SLIDING_MOVES_TABLE[index]


def get_bishop_moves(bitboard: int, square: int) -> int:
    ...
    return SLIDING_MOVES_TABLE[BISHOPS_START_FROM +index]

print(f"Length of table: {len(SLIDING_MOVES_TABLE)}")